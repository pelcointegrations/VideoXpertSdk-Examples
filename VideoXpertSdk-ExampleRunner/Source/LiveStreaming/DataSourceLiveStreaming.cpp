...
Plugin* ExampleRunner::LiveStreaming::DataSourceLiveStreaming::Run(DataModel* dataModel) {
    cout << "\n\n";
...
void ExampleRunner::LiveStreaming::DataSourceLiveStreaming::DisplayPlayerOptionsToConsole(bool isPtzEnabled, bool isLive, bool isRecording, bool isPlaybackMode) {
    ...
    if (isRecording) {
        cout << " 'P' to toggle between PAUSE and PLAY\n";
    }
...
// Streams a video in live.
void ExampleRunner::LiveStreaming::DataSourceLiveStreaming::DoLiveStreaming(VxCollection<IVxDataSource**> dataSources) {
...
IController* ExampleRunner::LiveStreaming::DataSourceLiveStreaming::GetController(IVxDataSource* dataSource, IVxDataInterface* dataInterface) {
...
VxCollection<IVxDataSource**> ExampleRunner::LiveStreaming::DataSourceLiveStreaming::GetDataSources(IVxSystem* vxSystem) {
...
void ExampleRunner::LiveStreaming::DataSourceLiveStreaming::ShowDataSourceDetails(IVxDataSource* dataSource) {
    // Read PTZ info
    bool canPtz;
...
bool ExampleRunner::LiveStreaming::DataSourceLiveStreaming::StartStreamingForDataSource(
    ...
    bool isPlaybackMode = seekTime != 0;
    char option;
    ...
    do {
        ...
        MediaController::IStream::Mode mode = mediaOperationshandler.GetMode();
        bool isLive = mode == MediaController::IStream::Mode::kLive;

        // Display the user options while streaming
        DisplayPlayerOptionsToConsole(isPtz, isLive, isRecording, isPlaybackMode);
        ...
        if (!isPtzCommandProcessed) {
            MediaController::IStream::Mode lastMode = mediaOperationshandler.GetMode();
            isPtzCommandProcessed = mediaOperationshandler.DoOperation(option, isLive, isRecording, isPlaybackMode);
            if (isPtzCommandProcessed && lastMode != mediaOperationshandler.GetMode() && dataInterface->protocol != VxStreamProtocol::kMjpegPull) {
...
void ExampleRunner::LiveStreaming::DataSourceLiveStreaming::TimestampCallback(TimestampEvent* timeEvent) {
