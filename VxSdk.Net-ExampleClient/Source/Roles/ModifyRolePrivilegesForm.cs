...
namespace ExampleClient.Source
{
    ...
    public partial class ModifyRolePrivilegesForm : Form
    {
        ...
        private IEnumerable<ComboBox> ComboBoxes => tlpLeft.Controls.OfType<ComboBox>().Concat(tlpRight.Controls.OfType<ComboBox>());
        ...
        private static void TableLayoutPanel_CellPaint(object sender, TableLayoutCellPaintEventArgs args)
        {
            if (args.Row % 2 != 0)
                return;
        ...
        private void HandleRelations(ListControl comboBox)
        {
            ...
            switch (permission)
            {
                case Privilege.PermissionIds.ViewLiveMedia:
                    if (!enabled)
                    {
                        cbxLaunchSavedTabsRemotely.SelectedIndex = 0;
                        cbxRecordMedia.SelectedIndex = 0;
                        cbxUsePtzMode.SelectedIndex = 0;
                    }

                    cbxLaunchSavedTabsRemotely.Enabled = enabled;
                ...
                case Privilege.PermissionIds.ViewRecordedMedia:
                    if (!enabled)
                    {
                        ...
                        cbxSystemBookmarks.SelectedIndex = 0;
                        cbxExportMediaClips.SelectedIndex = 0;
                        cbxManageExports.SelectedIndex = 0;
                    }

                    cbxExportMediaClips.Enabled = enabled;
                    cbxSystemBookmarks.Enabled = enabled;
                    cbxExportMediaClips.Enabled = enabled;
